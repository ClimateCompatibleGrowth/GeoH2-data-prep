...
data_path = 'C:\\Users\\lahert5576\\PycharmProjects\\GeoNH3\\Raw_Spatial_Data\\'
regionPath = data_path + 'ne_50m_admin_0_countries/ne_50m_admin_0_countries.shp'
clcRasterPath = data_path + "PROBAV_LC100_global_v3.0.1_2019-nrt_Discrete-Classification-map_EPSG-4326.tif"
oceanPath = data_path + "GOaS_v1_20211214_gpkg/goas_v01.gpkg"
protectedAreaPath = data_path + 'Africa_protect_class_1.geojson'
...
for country_name in country_names:
    ...
    with open(glaes_output_dir + f'{country_name}_EPSG.pkl', 'wb') as file:
       pickle.dump(EPSG, file)

    # reproject country to UTM zone
    country.to_crs(epsg=EPSG, inplace=True)
    country.to_file(glaes_output_dir + f'{country_name}.geojson', driver='GeoJSON', encoding='utf-8')
    ...
    country_buffer.to_file(glaes_output_dir + f'{country_name}_buff.geojson', driver='GeoJSON', encoding='utf-8')
    ...
    GOAS_country.to_file(glaes_output_dir + f'{country_name}_oceans.geojson', driver='GeoJSON', encoding='utf-8')
    GOAS_country.to_file(spider_output_dir + f'{country_name}_oceans.gpkg', driver='GPKG', encoding='utf-8')
    ...
    protected_areas.to_file(glaes_output_dir + f'{country_name}_protected_areas.geojson', driver='GeoJSON', encoding='utf-8')
    ...
    OSM_waterbodies = gpd.read_file(data_path + 'OSM\\' + f'{country_name}\\gis_osm_water_a_free_1.shp')
    OSM_waterbodies.to_file(spider_output_dir + f'{country_name_nospace}_waterbodies.gpkg', driver='GPKG', encoding='utf-8')
    OSM_roads = gpd.read_file(data_path + 'OSM\\' + f'{country_name}\\gis_osm_roads_free_1.shp')
    OSM_roads.to_file(spider_output_dir + f'{country_name_nospace}_roads.gpkg', driver='GPKG', encoding='utf-8')
    OSM_waterways = gpd.read_file(data_path + 'OSM\\' + f'{country_name}\\gis_osm_waterways_free_1.shp')
    OSM_waterways.to_file(spider_output_dir + f'{country_name_nospace}_waterways.gpkg', driver='GPKG', encoding='utf-8')
    ...
    country.to_file(spider_output_dir + f'{country_name_nospace}.gpkg', driver='GPKG', encoding='utf-8')
    ...
    with rasterio.open(clcRasterPath) as src:
        ...
        with rasterio.open(glaes_output_dir + f'{country_name}_CLC.tif', 'w', **out_meta) as dest:
